...
# node class with arguments for the weight, benefit, parent, depth, and included(True or false wether a node is included in
#the solution or not)
class node:
    def __init__(self, weight, benefit, parent, depth, included):
...
def knapsack(items, capacity):
    ...
    max_benefit = 0
    best_solution = None
# When adding the new nodes to the stack, DFS explores the most recently added node first, meaning
# that it goes deeper into the tree before backtracking to explore other branches. This is the depth-first traversal behavior.
    while stack:
        current = stack.pop()
        ...
        stack.append(new_node) # put the new node in the queue as it is included in the path/solution
        ...
        stack.append(new_node) # put the new node in the queue as it is not included in the path/solution
